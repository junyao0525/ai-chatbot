...
const MarkdownRenderer = React.memo(({ content }: { content: string }) => {
  // Define custom renderer with proper types - memoize the renderer object
  const renderer = React.useMemo(
    () => ({
      code(code: string, language?: string) {
        return (
          <pre className="bg-neutral-200 dark:bg-neutral-800 p-2 rounded-md overflow-x-auto my-3">
            <code className="text-neutral-800 dark:text-neutral-300 text-xs">
              {code}
            </code>
          </pre>
        );
      ...
      codespan(code: string) {
        return (
          <code className="bg-neutral-200 dark:bg-neutral-800 px-1 py-0.5 rounded text-xs">
            {code}
          </code>
        );
      },
...
export const ReasoningPartView: React.FC<ReasoningPartViewProps> = React.memo(
  ({
    ...
    const hasNonEmptyDetails =
      part.details &&
      part.details.some(
        (detail) => detail.type === "text" && !isEmptyContent(detail.text)
      );
    const hasNonEmptyReasoning =
